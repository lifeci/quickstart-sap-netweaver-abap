...
then
	...
	if [ $? -ne 0 ]
	then
 		#RJ-debug# /root/install/signalFinalStatus.sh 1 "Install-script did not execute correctly...check: /var/log directories for error message"
		echo "#RJ-debug#: Install-script did not execute correctly...check: /var/log directories for error message"
		echo "#RJ-debug#: sleep 7200s"; sleep 7200;
